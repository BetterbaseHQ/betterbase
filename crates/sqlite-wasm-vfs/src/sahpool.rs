        ...
        while let Ok(future) = iter.next() {
            ...
            if kind.as_deref() == Some("file") {
                ...
                if clear_files {
                    ...
                    let mut map = self.map_filename_to_file.borrow_mut();
                    if map.contains_key(&filename) {
                        // Duplicate filename detected. Keep the file with more
                        // data (larger size = more recent writes), clear the
                        // stale one's header and return it to the available pool.
                        let existing = map.get(&filename).unwrap();
                        let existing_size = existing.handle.get_size().unwrap_or(0.0);
                        let new_size = file.handle.get_size().unwrap_or(0.0);
                        if new_size > existing_size {
                            // New file is larger — swap it in, clear old
                            let old = map.insert(filename, file).unwrap();
                            drop(map);
                            clear_file(old)?;
                        } else {
                            // Existing is larger or equal — keep it, clear new
                            drop(map);
                            clear_file(file)?;
                        }
                    } else {
                        map.insert(filename, file);
                    }
                } else {
                    clear_file(file)?;
                }
