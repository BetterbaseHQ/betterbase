...
use betterbase_db::collection::builder::CollectionDef;
...
fn validate_sql_identifier(name: &str, context: &str) -> betterbase_db::error::Result<()> {
...
impl WasmSqliteBackend {
    ...
    fn borrow_conn(&self) -> betterbase_db::error::Result<std::cell::Ref<'_, Connection>> {
    ...
    pub fn close(&self) -> betterbase_db::error::Result<()> {
    ...
    fn read_record(stmt: &RawStatement<'_>) -> betterbase_db::error::Result<SerializedRecord> {
    ...
    fn execute_put_inner(&self, record: &SerializedRecord) -> betterbase_db::error::Result<()> {
        let conn = self.borrow_conn()?;
    ...
    ) -> betterbase_db::error::Result<Option<(String, Vec<SqlParam>)>> {
    ...
    fn query_records(
        ...
        sql: &str,
        params: &[SqlParam],
    ) -> betterbase_db::error::Result<Vec<SerializedRecord>> {
        let conn = self.borrow_conn()?;
    ...
    fn execute_index_scan_inner(
        ...
        scan: &IndexScan,
        index_provides_sort: bool,
    ) -> betterbase_db::error::Result<Option<Vec<SerializedRecord>>> {
        let Some((sql, params)) =
...
impl StorageBackend for WasmSqliteBackend {
    fn get_raw(
        ...
        collection: &str,
        id: &str,
    ) -> betterbase_db::error::Result<Option<SerializedRecord>> {
        let conn = self.borrow_conn()?;
        let sql = format!(
    ...
    fn count_raw(&self, collection: &str) -> betterbase_db::error::Result<usize> {
        let conn = self.borrow_conn()?;
        let mut stmt = conn
    ...
    fn batch_put_raw(&self, records: &[SerializedRecord]) -> betterbase_db::error::Result<()> {
    ...
    ) -> betterbase_db::error::Result<usize> {
        let conn = self.borrow_conn()?;
    ...
    fn get_meta(&self, key: &str) -> betterbase_db::error::Result<Option<String>> {
        let conn = self.borrow_conn()?;
        let mut stmt = conn
    ...
    fn set_meta(&self, key: &str, value: &str) -> betterbase_db::error::Result<()> {
        let conn = self.borrow_conn()?;
        let mut stmt = conn
    ...
    fn transaction<F, T>(&self, f: F) -> betterbase_db::error::Result<T>
    where
        F: FnOnce(&Self) -> betterbase_db::error::Result<T>,
    {
    ...
    fn scan_index_raw(
        ...
        collection: &str,
        scan: &IndexScan,
    ) -> betterbase_db::error::Result<Option<RawBatchResult>> {
    ...
    fn count_index_raw(
        ...
        collection: &str,
        scan: &IndexScan,
    ) -> betterbase_db::error::Result<Option<usize>> {
    ...
    fn scan_all_meta(&self) -> betterbase_db::error::Result<Vec<(String, String)>> {
        let conn = self.borrow_conn()?;
        let mut stmt = conn
