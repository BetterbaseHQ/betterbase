...
struct MockAdapterInner {
    ...
    mark_synced_response:
        Option<Box<dyn Fn(&str, &str, i64) -> betterbase_db::error::Result<()> + Send + Sync>>,
...
impl MockAdapter {
    ...
    fn on_mark_synced(
        &self,
        f: impl Fn(&str, &str, i64) -> betterbase_db::error::Result<()> + Send + Sync + 'static,
    ) {
...
impl SyncAdapter for MockAdapter {
    fn get_dirty(&self, def: &CollectionDef) -> betterbase_db::error::Result<BatchResult> {
    ...
    fn get_last_sequence(&self, collection: &str) -> betterbase_db::error::Result<i64> {
        let inner = self.inner.lock();
