use betterbase_db::schema::node::{created_at_schema, key_schema, t, updated_at_schema};
use betterbase_db::schema::validate::{
...
fn validate_or_throw_returns_less_db_error_on_failure() {
    ...
    assert!(matches!(err, betterbase_db::error::LessDbError::Schema(_)));
}
