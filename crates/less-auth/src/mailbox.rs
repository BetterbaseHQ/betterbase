...
const MAILBOX_SALT: &[u8] = b"less-platform-mailbox-salt-v1";
...
mod tests {
    ...
    fn known_test_vector() {
        ...
        let key = [0u8; 32];
        let id = derive_mailbox_id(&key, "https://accounts.less.so", "test-user").unwrap();
        assert_eq!(id.len(), 64);
        assert!(id.chars().all(|c| c.is_ascii_hexdigit()));
        // Pin the actual value so regressions are caught
        assert_eq!(
            id,
            "48bc2d7904de9399985175d243be74b6081921b9d0ea19529806bc2054c06fad"
        );
    }
}
