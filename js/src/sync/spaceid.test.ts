...
describe("personalSpaceId", () => {
  it("matches Go server known vector", async () => {
    ...
    expect(id).toBe("cf78fa75-e714-5073-8972-126a66255b39");
  });

  it("is deterministic", async () => {
    const id1 = await personalSpaceId("https://issuer.example.com", "user-123", "client-abc");
    const id2 = await personalSpaceId("https://issuer.example.com", "user-123", "client-abc");
    expect(id1).toBe(id2);
  });

  it("produces different IDs for different inputs", async () => {
    const base = await personalSpaceId("https://issuer.example.com", "user-123", "client-abc");

    const diffIssuer = await personalSpaceId("https://other.example.com", "user-123", "client-abc");
    const diffUser = await personalSpaceId("https://issuer.example.com", "user-456", "client-abc");
  ...
  it("produces a valid UUID v5", async () => {
    const id = await personalSpaceId("https://test.example.com", "user-1", "client-1");
